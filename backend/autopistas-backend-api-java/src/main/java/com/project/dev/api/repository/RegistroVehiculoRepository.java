/*
 * @fileoverview    {RegistroVehiculoRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code RegistroVehiculoRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface RegistroVehiculoRepository extends JpaRepository<RegistroVehiculo, Long> {
    ...
    @Query("SELECT m FROM RegistroVehiculo m WHERE m.intTiqueteNro LIKE CONCAT('%', :strSearch, '%')")
    public List<RegistroVehiculo> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM RegistroVehiculo m WHERE m.intTiqueteNro LIKE CONCAT('%', :strSearch, '%')")
    public Page<RegistroVehiculo> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
