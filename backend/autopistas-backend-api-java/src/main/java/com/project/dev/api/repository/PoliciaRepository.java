...
/**
 * TODO: Description of {@code PoliciaRepository}.
 *
 * @author Dyson Parra
 * @since 11
 */
...
public interface PoliciaRepository extends JpaRepository<Policia, Long> {
    ...
    @Query("SELECT m FROM Policia m WHERE m.intIdPolicia LIKE CONCAT('%', :strSearch, '%')")
    public List<Policia> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Policia m WHERE m.intIdPolicia LIKE CONCAT('%', :strSearch, '%')")
    public Page<Policia> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
