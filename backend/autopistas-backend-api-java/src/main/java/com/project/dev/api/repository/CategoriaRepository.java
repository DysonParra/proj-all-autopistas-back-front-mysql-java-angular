/*
 * @fileoverview    {CategoriaRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code CategoriaRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface CategoriaRepository extends JpaRepository<Categoria, Long> {
    ...
    @Query("SELECT m FROM Categoria m WHERE m.intIdCategoria LIKE CONCAT('%', :strSearch, '%')")
    public List<Categoria> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Categoria m WHERE m.intIdCategoria LIKE CONCAT('%', :strSearch, '%')")
    public Page<Categoria> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
