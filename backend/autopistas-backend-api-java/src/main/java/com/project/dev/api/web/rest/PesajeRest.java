/*
 * @fileoverview    {PesajeRest} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code PesajeRest}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public class PesajeRest {
    ...
    public CollectionModel<EntityModel<PesajeDTO>> getAllEntities() {
        ...
        try {
            ...
            CollectionModel<EntityModel<PesajeDTO>> entitiesCollection = new CollectionModel<>(entities);
            entitiesCollection.add(linkTo(methodOn(PesajeRest.class).getAllEntities()).withSelfRel());
    ...
    public ResponseEntity<List<PesajeDTO>> getAllEntitiesPaged(Pageable pageable) {
    ...
    @GetMapping("/Pesaje/{query}/pages")
    public ResponseEntity<List<PesajeDTO>> searchEntitiesPaged(@PathVariable String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/Pesaje/{query}/pages/" + query);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
