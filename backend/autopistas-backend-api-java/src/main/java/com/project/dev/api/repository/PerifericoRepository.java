/*
 * @fileoverview    {PerifericoRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code PerifericoRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface PerifericoRepository extends JpaRepository<Periferico, Long> {
     ...
     */
    public List<Periferico> findByIntId(Long id);
    ...
    @Query("SELECT m FROM Periferico m WHERE m.intId LIKE CONCAT('%', :strSearch, '%')")
    public List<Periferico> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Periferico m WHERE m.intId LIKE CONCAT('%', :strSearch, '%')")
    public Page<Periferico> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
