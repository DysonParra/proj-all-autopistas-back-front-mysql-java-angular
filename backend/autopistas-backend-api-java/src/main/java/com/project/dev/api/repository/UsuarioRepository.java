/*
 * @fileoverview    {UsuarioRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code UsuarioRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
    ...
    @Query("SELECT m FROM Usuario m WHERE m.intCedulaUsuario LIKE CONCAT('%', :strSearch, '%')")
    public List<Usuario> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Usuario m WHERE m.intCedulaUsuario LIKE CONCAT('%', :strSearch, '%')")
    public Page<Usuario> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
