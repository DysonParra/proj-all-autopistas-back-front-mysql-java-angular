/*
 * @fileoverview    {TransitoDinamicaRest} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code TransitoDinamicaRest}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public class TransitoDinamicaRest {
    ...
    public ResponseEntity<List<TransitoDinamicaDTO>> getAllEntitiesPaged(Pageable pageable) {
    ...
    @GetMapping("/TransitoDinamica/{query}/pages")
    public ResponseEntity<List<TransitoDinamicaDTO>> searchEntitiesPaged(@PathVariable String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/TransitoDinamica/{query}/pages/" + query);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
