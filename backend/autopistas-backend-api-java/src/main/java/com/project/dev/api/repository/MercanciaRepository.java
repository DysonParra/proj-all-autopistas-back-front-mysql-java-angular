/*
 * @fileoverview    {MercanciaRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code MercanciaRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface MercanciaRepository extends JpaRepository<Mercancia, Long> {
    ...
    @Query("SELECT m FROM Mercancia m WHERE m.intIdMercancia LIKE CONCAT('%', :strSearch, '%')")
    public List<Mercancia> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Mercancia m WHERE m.intIdMercancia LIKE CONCAT('%', :strSearch, '%')")
    public Page<Mercancia> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
