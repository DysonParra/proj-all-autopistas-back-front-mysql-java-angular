/*
 * @fileoverview    {PerifericoRest} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code PerifericoRest}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public class PerifericoRest {
    ...
    public CollectionModel<EntityModel<PerifericoDTO>> getAllEntities() {
        ...
        try {
            ...
            CollectionModel<EntityModel<PerifericoDTO>> entitiesCollection = new CollectionModel<>(entities);
            entitiesCollection.add(linkTo(methodOn(PerifericoRest.class).getAllEntities()).withSelfRel());
    ...
    public ResponseEntity<List<PerifericoDTO>> getAllEntitiesPaged(Pageable pageable) {
    ...
    @GetMapping("/Periferico/{query}/pages")
    public ResponseEntity<List<PerifericoDTO>> searchEntitiesPaged(@PathVariable String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/Periferico/{query}/pages/" + query);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
