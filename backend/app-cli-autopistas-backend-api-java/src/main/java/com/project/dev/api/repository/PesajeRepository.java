...
/**
 * TODO: Description of {@code PesajeRepository}.
 *
 * @author Dyson Parra
...
public interface PesajeRepository extends JpaRepository<Pesaje, Long> {
    ...
    @Query("SELECT m FROM Pesaje m WHERE m.intId LIKE CONCAT('%', :strSearch, '%')")
    public List<Pesaje> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Pesaje m WHERE m.intId LIKE CONCAT('%', :strSearch, '%')")
    public Page<Pesaje> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
